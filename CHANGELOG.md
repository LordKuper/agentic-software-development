...
- **Memory-finding routing** (`review-policy.md` "Autofix vs escalation"). A finding in `.claude/agent-memory/<owner>/` routes to `<owner>` in a fresh memory-fix dispatch: the owner fixes the file with its own write tool (on Claude a reviewer holds `Write` through `memory: project`), and the orchestrator commits it when the owner cannot; an owner with no write tool returns replacement text for the orchestrator to apply. `review-policy.md` "Gate Verdict Format" now states that policy, not the host, keeps a reviewer's writes to its own memory directory.
