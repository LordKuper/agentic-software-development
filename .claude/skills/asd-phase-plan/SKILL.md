...
4. Dispatch `asd-pm` via Task with payload:
   ...
   - instruction:
     ...
     - **Stub inclusion step** (before task decomposition):
       ...
       - if non-empty: AskUserQuestion per stub: include resolution this sprint / defer (leave open) / mark accepted-debt
       - per "include": add explicit `### Task N: Resolve stub <ref>` with owner derived from stub Owner column
       - per "accepted-debt": dispatch creator to edit stubs.md Reason field prepending `(accepted-debt)`
