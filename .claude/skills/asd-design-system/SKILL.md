...
description: "Forms or edits the project design system at design/ux/DESIGN.md, design/ux/design-system.html, and design/ux/accessibility.html. Reads the project concept and stack, then dispatches asd-ux-designer to author tokens, components, and accessibility baseline. Silent detection branches into one of three flows: (A) greenfield — designer proposes tokens from concept and stack; (B) constraints in mind — user supplies preferences (brand color, typography, density, platform) and designer proposes within them; (C) brownfield — extract tokens from existing CSS, component code, theme files, design exports, with provenance frontmatter. All flows fetch the current Google Labs DESIGN.md spec via WebFetch, lint via commands.yaml designmd-lint, regenerate design-system.html with live previews after every DESIGN.md change, and author accessibility.html covering visual/motor/cognitive/auditory/platform baselines. Section-by-section lock-in-or-revise loop with the user in language.chat; write approved version in language.docs. Re-runs detect existing DESIGN.md and enter diff-edit mode. Suggests next skill on handoff; does not auto-dispatch. Use when the user runs /asd-design-system, when asd-init or asd-phase-design detects missing DESIGN.md/design-system.html/accessibility.html and suggests this skill, or when the user asks to define, draft, refine, edit, augment, or reverse-engineer the project design system, design tokens, or accessibility baseline."
metadata:
  asd-role: artifact
  version: "0.1"
