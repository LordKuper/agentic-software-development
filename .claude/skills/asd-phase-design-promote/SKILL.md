...
- AskUserQuestion — rare, phase-level escalation only (PM + creators handle per-doc/per-subsystem approvals)
...
4. **PM orchestration** — dispatch `asd-pm` via Task:
   - update `state.json` (phase=design-promote)
   - if `subsystem_decomposition: enabled`:
     - propose overall per-subsystem split (which subsystems touched, which fragments go where); AskUserQuestion; iterate
     - detect new subsystems inferred from drafts; per each AskUserQuestion (name, parent container, description); collect approvals
