...
- Task — dispatch devs per task owner / finding owner / defect owner; PM for state + assessment + decisions-log
...
6. **Dispatch tasks** per execution graph:
   ...
   - per task: dispatch to assigned dev (`asd-backend-dev` | `asd-frontend-dev`; `asd-test-engineer` only for review findings in test files) via Task with payload:
...
9. **Impl completion gate** (all modes) — dispatch `asd-pm` via Task to verify, via `commands.yaml`:
...
10. **Impl assessment checkpoint** — **initial mode only** (fix modes skip to step 11) — dispatch `asd-pm` via Task:
   ...
   - present via AskUserQuestion: approve (advance to impl-test) / request changes / abort
   ...
   - on abort: emit ABORT
11. **Fix-mode finalize** — fix modes only — dispatch `asd-pm` via Task:
