...
- Editing a non-render canon file (e.g. `.asd/rules/*.md`, `.asd/workflows/*.md`, `.asd/migrations/*.js`, `.asd/templates/t_*` EXCEPT `t_AGENTS.md`/`t_CLAUDE.md`, which do render the `AGENTS.md`/`CLAUDE.md` managed blocks — tracked by `managed_paths`/`upstream_hashes` only, no generated view of their own) still turns the `upstream_hashes` test red and so needs a ledger refresh, but no `--apply` at all: recompute just that one entry with `sync.sha256Hex(sync.readNormalized(path))` and edit the single line. Keeps the ledger diff scoped to your own change.
