...
The pins are not limited to rule docs and workflows: `.asd/agents/*.md` bodies are pinned too (an `asd-dev.md` Tool-policy grant is asserted verbatim, via `sync.readNormalized`, so the generated view is checked as well).

**How to apply:** before rewording any workflow/rule/agent sentence, grep `tests/run.js` for a distinctive phrase from the line. If it is pinned, build the new sentence *around* the pinned literal instead of replacing it, or escalate — never edit the assertion. Related: [[feedback_fix-the-class]].
