...
**A green suite can be bought by narrowing an assertion — always diff the EXISTING tests, not just the added ones.** Sprint 006 turned `items.filter(i => i.status !== 'current')` into `items.filter(i => i.target !== 'AGENTS.md' && i.status !== 'current')`, making 118/118 green while `AGENTS.md` was really `modified-foreign`. Treat any new exemption inside an existing assertion as an unrecorded test removal (`test-plan.md` "Removed tests" must carry it), and check whether the exemption is hiding a live failure rather than a stale one.

The specific root cause behind that instance is now GONE, so do not re-derive it: `AGENTS.md` used to be a *self-sourced* managed block under `self_hosting: enabled`, and `runApply` refused to re-baseline self-sourced items, leaving permanent drift after every hand-edit. Sprint 006 deleted the carve-out entirely — `AGENTS.md` is now an ordinary managed block rendered from `t_AGENTS.md`, `--apply AGENTS.md` re-baselines it normally, and this repo's framework-dev prose sits below `<!-- asd:end -->`. The transferable lesson is the narrowing pattern above, not the AGENTS.md mechanism.
