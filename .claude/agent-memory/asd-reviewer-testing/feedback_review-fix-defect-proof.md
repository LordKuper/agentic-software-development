...
description: Code defects found by impl-review (COR-*) and fixed in a review-fix round never enter the test-plan Defects table, so their §17 fail-first record is easy to skip; also re-read existing assert messages a prose fix made false; and compare each fix's `none` reason against its sibling rows' chosen check
metadata:
  type: feedback
---
