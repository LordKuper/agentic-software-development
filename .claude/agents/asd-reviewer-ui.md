---
# ASD generated. Edit .asd/agents/asd-reviewer-ui.md. source_digest=sha256:0997ec7ba29b9b944925185566f13ae3cee03522a9c499440ef70d63a15e5282 content_digest=sha256:f4656d8b7689f5847fd60b9e31d1b6d6b6128b62ffe0e0d34cc8f9318b50e3de asd_version=2.0.0 schema=1
...
**Self-hosting framework-templates carve-out reduced rubric**: when reviewing under the Stop-conditions carve-out (2) above, **Token comment** (§4) and **Lint exclusions** (§11) are n/a — no DESIGN.md/designmd-lint pipeline exists for framework templates; note both as n/a in the coverage ledger, not as findings. All other rubric items apply, substituting WCAG AA thresholds for the missing accessibility.html and `design-system.md`/`ux-principles.md` for the missing DESIGN.md/ux-spec — **except Token usage (§6)**: `t_html-shell.html`'s `:root` and `prefers-color-scheme` blocks are the primitive-layer token *definitions* §6 already exempts; check §6 only against everything OUTSIDE those two blocks (i.e. that consuming rules reference `var(--*)`), never flag the definitions themselves.
