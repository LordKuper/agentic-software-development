...
- AC-8: In both review phases (`design-review` and `impl-review`), for every reviewer agent — the internal reviewers and External Review — the orchestrator builds the explicit list of files that agent must review, per wave and per part where these apply. That per-agent list is the agent's **only** scope source. The agent never derives, widens or narrows its scope itself (from git, a diff range, `exclude_paths` or any other input); a diff or ref may be supplied only as context for the listed files. The mechanism is stated once in one canonical home; every workflow, agent file and `external-review.md` links to it instead of restating it.

## Out of scope
- Waves for `design-review`: its part splitting and dispatch stay as they are (AC-8 still applies to it).
