...
## 2026-09-09 — PR #33 opened

- **Decision**: Branch pushed and PR #33 opened against `main` after explicit user approval. `state.json.pr` records number, url and `state: "open"`. The sprint is not archived and not done — merge and closure are separate steps, and closure is a hard gate no publication or merge can satisfy.
- **Rationale**: Every DoD input was confirmed before publication: all plan tasks ticked, all ten acceptance criteria traced, all five reviewer verdicts APPROVE at iteration 5, terminal full suite green at `d76482b` with 184/184, lint and build clean, no open stub, retrospective written. Version bumped 7.1.0 → 7.2.0 as MINOR — `feat` commits present, nothing breaking, and the plan format's new required section carries a fallback for plans authored before the rule — with the migration-version check satisfied (max migration 6.0.0 ≤ 7.2.0).
