...
## 2026-09-08 — impl-review iter-05: four APPROVE, external CONCERNS on a verified critical finding

- **Decision**: correctness, efficiency and testing returned APPROVE at the `critical` floor and latched; documentation was latch-skipped from iteration 4 and inherits its APPROVE. External Review is recorded **CONCERNS** with one critical finding, and the sprint routes to `impl` review-fix.
- **The external dispatch history, and why CONCERNS**: the first dispatch returned no verdict and was recorded as an interrupted attempt and re-dispatched. Both runs then delivered — the re-dispatch `APPROVE`, the original `CONCERNS` with a critical finding. The orchestrator verified the finding on disk before recording: `providers.md` claims universally that reviewer agents carry no artifact-write grant and that this is "enforced by config", but `asd-external-review` is a reviewer agent and carries `Bash`, so the claim is false for it and `README.md` already states the correct carved-out version. Recording the re-dispatch's `APPROVE` would ship a known-false, security-relevant claim in an always-loaded doc. The two runs do not actually disagree about the state: the `APPROVE` run flagged the same defect class as `high`, dropped only because the floor is `critical`.
- **Rationale**: the finding is in AC-15's own scope — reconciling the reviewer read-only claim with what the host actually grants — so the criterion was delivered incompletely rather than wrongly. The dispatch bookkeeping determines which verdict is authoritative; it does not determine whether a verified defect exists.
- **Affected docs**: [reviews/impl/iter-05/](reviews/impl/iter-05/), [state.json](state.json)

## 2026-09-08 — impl fix for iter-05: both external findings resolved

- **Decision**: X-1 and X-2 are resolved and `review_fixes_pending` is cleared. `providers.md` now states the artifact-write claim with its one carve-out named — External Review needs `Bash` to invoke the wrapped CLI at all, so its guarantee is enforced on the wrapped subprocess — and the four reviewer bodies cite `review-policy.md` "Gate Verdict Format" for their write scope instead of restating an absolute. The correctness reviewer's memory file, which carried the same stale copy outside the iteration's scope, was corrected in the same pass.
- **Rationale**: the claim was true of the four internal reviewers and false of the fifth, so scoping it was the fix rather than weakening it. No tool grant changed — `Bash` is load-bearing for the external reviewer. The dev kept the literal opening clause the AC-15 test asserts on and qualified it in the same sentence, so the guard still holds against a future restatement.
...
## 2026-09-08 — PR #29 opened; asd_version 6.0.0 → 7.0.0

- **Decision**: `asd_version` bumped to 7.0.0 with a matching CHANGELOG section, and PR #29 opened against `main`. The sprint stays active at its normal path while the PR is open.
- **Rationale for MAJOR**: the sprint carries a `refactor(workflow)!` commit, which the SemVer rule maps to MAJOR on its own. It is also right on the merits — three consumer-visible breaks ship: a bare `--apply` now fails instead of reporting success, the reviewer read-only guarantee is restated in the narrower form that is actually true, and `derived_handoff` was added and retired within the release. The blocking migration check passes: `max(.asd/migrations/*.js) = 6.0.0 <= 7.0.0`, and no new migration script is needed because no consumer state changes shape.
- **DoD evidence, re-verified at `pr` entry** rather than carried from the terminal gate: 160/160 unscoped, `git diff --check` clean, `node .asd/sync.js --check` `ok: true`, all five iteration-6 verdicts APPROVE, zero unticked plan tasks, zero sprint-owned stubs, no pending defect, retrospective written. The only changes since the terminal gate were sprint bookkeeping.
- **Affected docs**: [state.json](state.json), [CHANGELOG.md](../../../CHANGELOG.md), [release-manifest.json](../../release-manifest.json)

## 2026-09-08 — sprint closure approved; terminal state and archival

- **Decision**: The user granted explicit closure approval after PR #29 was squash-merged as `e15929d`. Terminal state written — `phase="done"`, `pr.state="merged"`, `archived_at` set — and the sprint folder moved to `.asd/sprints/archived/008-retro-007-remediation/` on this companion branch, per `git-strategy.md` "Finalize after closure". The annotated tag `v7.0.0` and its release follow the companion PR's merge, never before.
- **Rationale**: closure is the one hard gate no adaptive pass can satisfy — neither PR publication, nor the merge, nor `auto_pr` substitutes for it. The evidence presented was the merged state on `main` (`asd_version` 7.0.0, suite 160/160), the AC disposition (12 delivered, AC-4 closed as already satisfied at audit, AC-11 retired at review iteration 4, AC-15 added under authorization), five APPROVE verdicts at iteration 6, and a retrospective derived from eight friction entries.
- **Affected docs**: [state.json](state.json), [retrospective.html](retrospective.html)
