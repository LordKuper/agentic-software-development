...
## 2026-09-02 — impl-review iter-5: CONCERNS(quality×1, simplification×2) + APPROVE(implementation, testing, documentation, performance, ui, external) → impl fix

- **Decision**: no FAIL, 6 of 8 reviewers APPROVE — severity floor was `critical` this iteration (tightest of the sprint), and only 2 reviewers found genuinely critical issues, both converging on the same file. Route to `impl` review-fix mode, `review_fixes_pending = "iter-5"`. No escalation.
- **Findings, all in `.asd/skills/asd-design-system/SKILL.md`, independently confirmed by external as below-floor items too**: (1) quality#1/critical — Phase 6's create-mode-only skeleton guard is keyed on the shared session-global mode flag (derived solely from DESIGN.md's existence at Phase 1), not on accessibility.html's own existence; since `asd-phase-design.md` dispatches this skill when ANY of the 3 files is missing, "DESIGN.md present + accessibility.html missing" is reachable and can result in `COMPLETED` without accessibility.html ever being created, or an unwrapped fragment written. (2) simplification#1/critical — option `C) Skip` in the Phase 4 (DESIGN.md) per-section loop has no removal handler — the iter-4 external#2 fix landed in 2 of 3 skills (asd-concept, asd-stack) but missed this file, despite the fix-round entry claiming all 3 were fixed. (3) simplification#2/critical — the Hard rule "design-system.html MUST be regenerated once, at Phase 5" now contradicts this round's own Phase 7 re-run requirement (re-run Phase 5 regen on Phase-4 re-entry) — same fact stated twice, diverged in cardinality.
...
## 2026-09-03 — pr open mode: DoD met, PR #19 opened

- **Decision**: Definition of Done verified green and PR [#19](https://github.com/LordKuper/agentic-software-development/pull/19) opened against `main` from `sprint/003-doc-links-and-autonomy`. Title: `feat: write-then-review-accept gates, asd-advisor agent, ADVICE_NEEDED signal`.
- **DoD evidence**: plan 68/68 subtasks checked across 13 tasks; AC-1..AC-8 fully traced by the iter-6 Implementation reviewer (sole owner of the AC→code trace; `documents.prd` disabled, so `sprint.md`'s own AC-N is the source); `reviews.impl.verdicts["iter-6"]` all 8 `APPROVE`; zero `003-doc-links-and-autonomy` rows in `.asd/project/stubs.md` and zero `// TODO(sprint-003-doc-links-and-autonomy):` markers; content-scoped diff `8567b47d2...HEAD` (excluding `.asd/sprints/**`, `.asd/project/**`) empty, so the recorded `Suite run` pass stands — re-ran regardless after the version bump: `node tests/run.js` 83/83, `node .asd/sync.js --check` `ok: true` 72/72 current.
- **Version**: `asd_version` 3.0.0 → 3.1.0 (MINOR). Highest Conventional Commit type on the branch is `feat`; no `!` marker, no `BREAKING CHANGE` footer. The dropped design-promote final-mutation/step-8 and `c4-full/` gates change workflow behavior but break no consumer-facing contract, config key, or artifact schema, so no migration path is owed. Matching `## v3.1.0` section added to `CHANGELOG.md`.
- **Gate provenance (recorded honestly)**: the `pr` opening confirmation is an approve-before-write HARD gate. The dispatched `asd-pm` session had no discrete user-decision tool in its toolset, so per `checkpoints.md` it did not downgrade the gate to inferred approval — it presented the composed title and body and halted. The user gave an explicit "Confirm — open PR" through the dispatching session. `gh pr create` was then denied twice by the auto-mode irreversible-action classifier (not a missing `settings.json` rule — `Bash(gh pr:*)` was already permitted; no settings change was made), so the user ran `gh pr create` themselves after leaving auto mode. `git push` of the branch itself was performed by the agent and succeeded. PM verified the resulting PR independently via `gh pr view 19` before writing this entry and `state.json.pr`.
- **Affected docs**: [state.json](./state.json), `CHANGELOG.md`, `.asd/release-manifest.json`
- **Alternatives considered**: routing around the classifier denial via `gh api` or a wrapped invocation — rejected outright as working around a permission denial rather than accomplishing the task; PM halted and reported instead.

## 2026-09-03 — sprint files archived pre-merge, pending PR #19 merge

- **Decision**: sprint folder moved `.asd/sprints/003-doc-links-and-autonomy/` → `.asd/sprints/archived/003-doc-links-and-autonomy/` and `archived_at` stamped. Not a new gate — bookkeeping that follows DoD + PR success per `asd-phase-pr.md` step 6.
- **Rationale**: the move lands as its own commit on the already-open PR branch, so it squash-merges atomically with the sprint's own changes and survives auto-delete-branch. `phase` stays `pr` and `pr.state` stays `open` — the sprint is not merged, only its folder has moved; it is still detected as active on the next `/asd-sprint` invocation.
- **Next**: `pr` merge mode on re-entry — confirm PR #19 merged via `gh pr view 19`, then set `pr.state="merged"` / `phase="done"` on this already-archived `state.json` (the one sanctioned write to an archived sprint), and cut the annotated tag `v3.1.0` plus `gh release create` from the `CHANGELOG.md` v3.1.0 section (self-hosting).
