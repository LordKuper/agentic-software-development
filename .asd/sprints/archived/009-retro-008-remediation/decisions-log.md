...
## 2026-09-09 — scope expansion: AC-19, the orchestrator merges the PR

- **Decision**: The user authorized, after PR #31 was opened, a rule change outside the accepted `AC-1`..`AC-18`: the main orchestrator merges the sprint PR itself. Recorded as `AC-19`. `git-strategy.md` gains "Merging a PR" as the sole home; `sprint-lifecycle.md` "PR phase", `asd-phase-pr.md` merge mode and the `README.md` phase table cite or mirror it. Ledger refreshed for the three ledgered files; suite 171/171 and `sync --check` re-run after the diff, per open mode step 1.
- **Rationale**: New scope is a hard gate in both policies, and the user's instruction is that authorization. The rule deliberately keeps merge and closure separate: merging ends the branch, closure ends the sprint, and only the latter is a user gate — collapsing them would let archival, terminal state and the release tag ride an action the orchestrator performs on its own. Forced merges are excluded in the same sentence: no `--admin`, no local merge pushed to the base branch.
- **Affected docs**: [sprint.md](sprint.md), `.asd/rules/git-strategy.md`, `.asd/rules/sprint-lifecycle.md`, `.asd/workflows/asd-phase-pr.md`, `README.md`

## 2026-09-09 — closure: a defect found at the fresh checkout, fixed on the companion branch

- **Decision**: The companion branch carries a test-side defect fix beyond terminal state and the archive move: the CRLF+BOM input is built at runtime inside the test, and the committed fixture `tests/fixtures/canon/agents/demo-agent.crlf-bom.md` is deleted as unused. Authorized by the user after the deviation was put to them with alternatives. Recorded as friction `F-7`.
- **Rationale**: `AC-7` normalised a fixture whose bytes ARE the test input; its CRLF had never been in the blob, only in what `core.autocrlf` produced at checkout. On a clean clone the suite fails 170/171, and the `v7.1.0` tag would have been created on that commit. Fixing it inside sprint 009 keeps the work within the sprint that introduced it — a standalone hotfix PR would be project work outside a sprint, which the hard rules forbid — at the cost of the companion PR carrying more than its contract names. The first fix attempted, and recorded here before it was tested, was a `-text` attribute plus CRLF bytes committed for real; it was rejected on measurement because it breaks `AC-7`s own assertion that no tracked blob is CRLF in the index, and makes the `AC-6` staged lint report every line of the fixture as trailing whitespace. Building the input at runtime keeps both invariants and removes the checkout dependency entirely. The retrospective is already written and does not cover `F-7`.
- **Affected docs**: tests/run.js, `tests/fixtures/canon/agents/demo-agent.crlf-bom.md` (deleted), [test-plan.md](test-plan.md), [friction-log.md](friction-log.md)
