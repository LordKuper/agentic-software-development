...
## 2026-09-07 — Impl-review iteration 3: verdicts and routing

- **Verdicts** (floor `high`, HEAD `08ea5d1`, incremental scope of 22 paths since `60a991a`): correctness CONCERNS:1 · efficiency **APPROVE** · testing **APPROVE** · documentation CONCERNS:1 · external `APPROVE (skipped: negative-cache — quota)`. Efficiency and testing latch at iteration 3; iteration 4 dispatches only correctness and documentation.
- **The negative cache paid for itself.** External Review was skipped with **no agent dispatched at all** — `external-preflight` returned `negative-cache` from the failure recorded in iteration 2, still inside its bounded TTL. Iterations 1 and 2 each spent an agent, a wrapped-CLI attempt and a retry to reach the same conclusion; this cost one local probe pair. First measurable saving from the mechanism this sprint added.
- **Two findings, both real, both introduced by the previous wave's own fixes**:
  - correctness F1 (high): `exclude_paths[]` was defined as "never read even if reachable another way", which contradicts the project-context reference paths the same files hand the reviewer. In consumer mode those sit under `docs/**`/`.asd/**` and under self-hosting the AC list is inside `.asd/sprints/**` — so by the literal contract the external impl reviewer cannot read the acceptance criteria at all, making its own "requirements without coverage" rubric item unexecutable. Resolution direction: separate "out of review scope" from "unreadable" in the rule doc, rather than adding a `reference_paths[]` field — the wording fix carries no new machinery.
  - documentation F1 (high): `README.md:217` still advertises the removed `files/commits` manifest shape and omits the now-required `exclude_paths[]`. The only place in the repo still describing a deleted contract field.
- **Orchestrator finding, flagged by two reviewers as out-of-scope**: deleting `isSelfHostingRepo` left `.asd/skills/asd-init/SKILL.md:23` and `.asd/skills/asd-update/SKILL.md:14` (plus four generated views) citing a function that no longer exists. Change-induced staleness in files outside the iteration scope — the documented exception to the change-surface rule. Joins the fix set.
- **Orchestrator error corrected**: the `Result (entry 6)` line was missing from `test-plan.md`'s Suite run section. The orchestrator's earlier edit had silently no-opped — its anchor string no longer matched after the tester rewrote that section — and the script reported success without verifying the substitution. Caught by the testing reviewer, now inserted with the insertion verified rather than assumed.
- **Routing**: `review_fixes_pending = "iter-03"`, phase `impl`. Iteration 4 keeps floor `high` (the `high` tier's budget covers iterations 3-4).

## 2026-09-07 — Impl fix for iter-03: findings resolved

- **One task, three items** — the fix set was small enough that partitioning by file ownership would have cost more than it saved.
- **correctness F1** resolved by wording, not machinery: `external-review.md` now defines `exclude_paths[]` as bounding what the reviewer *judges* (what may appear in `files[]`, where a finding may be located), explicitly not what it may *read*, with the prompt's named project-context reference paths readable regardless and never valid finding locations. The agent file and both prompt templates cite that one sentence rather than inventing their own. The rejected alternative was a `reference_paths[]` manifest field — declined because this finding exists precisely because a boundary rule was restated in four places and one over-reached; a fifth place to drift would treat the symptom.
- **documentation F1** resolved: README now describes the current manifest shape and defers to `external-review.md` instead of re-listing fields.
...
## 2026-09-07 — Impl-review iteration 4: DoD met, terminal full suite green

- **Verdicts** (floor `high`, 9-path incremental scope since `08ea5d1`): correctness **APPROVE** · documentation **APPROVE** · efficiency APPROVE (latched iter 3) · testing APPROVE (latched iter 3) · external `APPROVE (skipped: negative-cache — quota)`. Reviewer roster satisfied; correctness and documentation latch at iteration 4.
- **Only two reviewers were dispatched** this iteration — the latch skipped the other two entirely — against 9 files instead of the 59 iteration 1 reviewed. Scope and roster both narrowed as the loop converged, which is what the latch and incremental scoping exist to do.
- **Documentation judged the four-site restatement rather than counting it**: the prompt templates legitimately carry the sentence because a prompt is a runtime payload piped into another provider's CLI over stdin, where a link is not a usable carrier. That is the allowed-mirror case, not a second home. The distinction matters — a mechanical SSoT rule would have demanded a fix that broke the transport.
- **Terminal full-suite gate (step 9), unscoped, dispatched to `asd-tester`** at `d7b0ac4`: `node tests/run.js` 127/127 exit 0; `lint` (`git diff --check`) clean; `build` (`node .asd/sync.js --check`) `{"ok": true}`, all items current. **GREEN** — no test defect, no code defect, no `D-N` row, no in-phase fix.
- **Orchestrator error corrected**: several earlier notes in this session claimed the repo defines no `lint`/`build` commands. Both exist in `.asd/project/commands.yaml` — `git diff --check` and `node .asd/sync.js --check`. The claim was inferred from the absence of a compiler and linter instead of read from the file.
- **Routing**: impl-review DoD met → `NEXT: pr`. Both fix flags null.
- **Carried risk for the pr phase**: External Review never ran this sprint — four consecutive availability skips on one Codex account limit. The scope-manifest transport it was meant to exercise was introduced and twice corrected during the sprint, both times by internal review reading the contract rather than by running it. It must be either validated end-to-end before closure or recorded as an explicitly accepted residual risk.

## 2026-09-07 — PR open mode: DoD verified, residual risk accepted

- **DoD checks, all passing**: 21/21 plan tasks ticked, 0 unticked; `.asd/project/stubs.md` holds no open stub; reviewer roster satisfied at iteration 4 with every verdict recorded; terminal full suite green at `d7b0ac4` (127/127, `lint` and `build` both clean); `asd_version` is `5.0.0` and the highest migration filename is `5.0.0.js`, so the self-hosting migration-reachability guard restored by correctness F3 passes.
- **Changelog completed before composing the PR**: v5.0.0 was missing four consumer-facing changes landed late in the sprint — the scope-manifest transport replacing the rendered diff, the canonical gitignored preflight cache path, and the paired `impl-review` clean-worktree precondition / `impl-test` commit obligation. Added under Added and Changed.
- **Accepted residual risk (decision_actor=user)**: the PR opens with External Review never having run this sprint. Four consecutive iterations skipped on one Codex account quota limit — iterations 1 and 2 after a real attempt, iterations 3 and 4 via the negative cache without dispatching an agent. The scope-manifest transport this sprint introduced was corrected twice during the sprint, both times by internal review reading the contract rather than by executing it, so its end-to-end behaviour against a live wrapped CLI is unproven.
