...
3a. Seed the remaining state placeholders: `{{USER_GATES}}` from `config.user_gates` (accept only `adaptive|strict`; absent -> `strict`); `{{DOC_PRD}}`/`{{DOC_UX_SPEC}}`/`{{DOC_ADR}}`/`{{DOC_C4}}` from normalized `documents.*` (absent group -> all enabled); `{{SKIP_DESIGN_PHASES}}` from `config.skip_design_phases` (accept only `enabled|disabled`; absent -> `disabled`). When it freezes `true`, write `{{DOC_PRD}}`/`{{DOC_UX_SPEC}}`/`{{DOC_ADR}}`/`{{DOC_C4}}` as `false`; if any of them would otherwise be `true`, append one decisions-log line naming those documents as suppressed by `skip_design_phases`. No placeholder literal may survive the write.
