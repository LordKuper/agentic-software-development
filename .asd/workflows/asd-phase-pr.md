...
1. Read config, state, plan, reviews, test-plan, retrospective and stubs. Confirm every plan task, AC trace, required review verdict (satisfied per `sprint-lifecycle.md` "State recovery", External Review's skip and partial forms included), full-suite record, lint/build record and stub rule; `pr` requires review DoD plus a completed `retro` (`checkpoints.md`), so `<sprint>/retrospective.html` is a DoD input and its absence blocks. Re-run required checks after a relevant diff. A failed or missing check blocks.
