...
4. **DoD verification** — delegate to agent `asd-pm` with payload (config, sprint paths, stubs path, commands.yaml):
   ...
   - **Tests pass / Lint clean**: compare current `git rev-parse HEAD` against the `HEAD` field recorded in `<sprint>/test-plan.md`'s `Suite run` section (the exact commit sha the suite was verified at, recorded by impl-test step 7). Equal (HEAD hasn't moved) → trust the recorded `Suite run` pass, skip re-running; impl-review produces no code/test/stub changes per its own workflow contract, so an intervening impl-review phase cannot invalidate an already-green suite. Different (HEAD moved — e.g. a fix commit landed after the last recorded run) → run command `commands.yaml` `test` then `lint`; non-zero exit on either = block
