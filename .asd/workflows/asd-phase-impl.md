...
2a. Before each task dispatch, run `node .asd/runtime.js route-task --input <path>` with kind, objective inputs/checks, risks, correction attempts and prior tier. A `command` result with `deterministic-state` runs directly; all other work uses the returned named agent variant. Persist tier, reason and selector. Invalid routing blocks; tier never lowers.
