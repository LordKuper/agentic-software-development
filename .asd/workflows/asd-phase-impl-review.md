...
6. **Parallel dispatch** — every reviewer delegated to as a **fresh agent** each iteration (clean-context dispatch per `review-policy.md`); no reviewer reused across iterations; every reviewer below is always dispatched (no agent-level skip — see step 5 for the section-level n/a mechanism):
...
8. Parse first-line verdict tokens from all written reviewer files; record per-reviewer verdicts under `state.json` `reviews.impl.verdicts["iter-NN"]` keyed `correctness`/`efficiency`/`testing`/`documentation`/`external`; aggregate per `sprint-lifecycle.md` "State recovery" satisfied-vs-blocking semantics. impl-review does NOT fix findings itself — fixes route to impl phase (fix mode):
   - **All APPROVE** → DoD met:
...
## Artefacts produced
- `<sprint>/reviews/impl/iter-NN/correctness.md` (written by this workflow, reduced coverage form of the reviewer's returned text; UI conformance section may be n/a per step 5's diff-scoped fan-out)
- `<sprint>/reviews/impl/iter-NN/efficiency.md` (written by this workflow; the five performance sections may be n/a per step 5's diff-scoped fan-out)
...
## Agents delegated to
- 4 internal reviewers (Correctness, Efficiency, Testing, Documentation) — parallel, always dispatched; `review.scoped_fan_out` degrades a rubric section within Correctness or Efficiency to `n/a`, never skips the agent (step 5)
