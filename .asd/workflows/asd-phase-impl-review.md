...
5. **Diff-scoped fan-out** (verdicts P-9/AG-11/AG-14, gap G-12, risk R-10 — every mitigation below is mandatory, not optional) — before dispatch, compute two skip predicates from the iteration's **scope file list only** (the diff's changed files); NEVER from `documents.*` (preserves `review-policy.md` line 136 verbatim: "absence of a ux-spec draft never implies absence of UI code to review"):
   ...
   - `review.scoped_fan_out: enabled` (default) → apply the two conditional skips below:
     ...
     - A skipped reviewer is **not dispatched**. In its place, write `state.json.reviews.impl.verdicts["iter-NN"].ui` or `.performance` = `"skipped: <predicate>"` (gap G-12; e.g. `"skipped: no UI surface in scope"`, `"skipped: no perf budgets section and no executable file in scope"`) instead of a parsed verdict token. No review file is written for a skipped reviewer. This value is distinct from an absent key (dispatch lost/crashed/ledger-rejected) and from `null` — see `sprint-lifecycle.md` "State recovery". Per `review-policy.md` line 136 / DoD table, a skipped reviewer is not counted as missing at the DoD or pr-phase gate.
