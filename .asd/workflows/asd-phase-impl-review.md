...
- **Clean worktree at phase entry** (`sprint-lifecycle.md`'s impl-review contract, sole SSoT — not restated here): before any dispatch, run `git status --porcelain`; non-empty output → `FAILED`, naming the dirty paths. Measured once, at entry, before step 1 — the phase's own subsequent writes (review files, `state.json`, `decisions-log.md`, `test-plan.md`) are produced after this gate and never re-trigger it.
