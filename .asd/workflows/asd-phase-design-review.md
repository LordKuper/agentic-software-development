...
7. **Parallel dispatch** — every reviewer delegated to as a **fresh agent** each iteration (clean-context dispatch per `review-policy.md`); no reviewer reused across iterations:
   ...
   - `asd-reviewer-correctness` — always dispatched (unless latch-skipped); its **allowed-section list this phase is UI conformance only** (ux-spec compliance with DESIGN.md + accessibility.html), and even that section applies **only when `ux-spec.html` or design-system artifacts are in the draft set this iteration** — otherwise the UI section is marked `n/a: no ux-spec/design-system draft in scope` in its section-coverage ledger, every impl-only section (Bugs, Security, Contracts, Best practices, AC coverage trace) is `n/a: outside phase gate`, and the reviewer still returns one verdict covering Documentation/Efficiency-adjacent n/a bookkeeping only — i.e. this reviewer's verdict is APPROVE whenever its (empty) allowed scope has no findings
...
9. Parse first-line tokens from all written reviewer files; record per-reviewer verdicts under `state.json` `reviews.design.verdicts["iter-NN"]` keyed `correctness`/`efficiency`/`documentation`/`external`, one entry per reviewer actually dispatched this iteration — a latch-skipped reviewer (step 7) gets no entry here. For any reviewer whose parsed token this iteration is `APPROVE`, also write `state.json.reviews.design.latched[<key>] = N` (current iteration; AC-2 APPROVE latch — `sprint-lifecycle.md` "APPROVE latch"). Aggregate, where a `latched` entry counts as satisfied exactly as a fresh `APPROVE` would:
...
- `<sprint>/reviews/design/iter-NN/correctness.md` (written by this workflow when dispatched; UI section n/a when no ux-spec/design-system draft is in scope this iteration — step 7; not written this iteration when latch-skipped)
...
## Agents delegated to
- 3 internal reviewers (Documentation, Efficiency, Correctness) — parallel, dispatched for any non-empty draft set unless APPROVE-latched (step 7, AC-2); Correctness's UI section (its only design-review section) is n/a when no ux-spec/design-system draft is in scope this iteration (step 7) — a dispatched agent is never section-skipped down to zero work, only the latch skips the whole agent
