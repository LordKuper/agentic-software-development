...
9. Parse first-line tokens from all written reviewer files; record per-reviewer verdicts under `state.json` `reviews.design.verdicts["iter-NN"]` keyed `correctness`/`efficiency`/`documentation`/`external`, one entry per reviewer actually dispatched this iteration — a latch-skipped reviewer (step 7) gets no entry here, and Correctness gets no entry either on an iteration where step 7's non-latch skip fired (no ux-spec/design-system draft in scope). For any reviewer whose parsed token this iteration is the bare `APPROVE` token — a completed review — also write `state.json.reviews.design.latched[<key>] = N` (current iteration; `sprint-lifecycle.md` "APPROVE latch"). External Review's availability-skip verdict is recorded instead as `"APPROVE (skipped: <reason>)"` (never the bare token) when its wrapped-CLI probe fails (`external-review.md` "Detection") — it satisfies the aggregation below exactly like a bare `APPROVE` but is NEVER written to `latched` (`sprint-lifecycle.md` "APPROVE latch" "Availability-skip carve-out" — only a verdict from an actual review latches). Aggregate, where a `latched` entry counts as satisfied exactly as a fresh `APPROVE` would:
