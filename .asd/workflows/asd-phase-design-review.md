...
8a. **Interrupted dispatch / split dispatch** — `review-policy.md` "Interrupted dispatch and split dispatch" is the sole SSoT for trigger, partition, union property, merge rule and durable record, not restated here. The step header carries no reach; each branch below states its own.
   - Interrupted dispatch — applies to any dispatch, External Review included: a dispatch returning no verdict token or no ledger takes the same reject-and-re-dispatch-fresh path as a failed validation above. An External Review return that is neither a verdict nor an availability skip is exactly this case per `external-review.md` "Outcome contract" (which imports that rule whole) — re-dispatched fresh and its attempt logged, never split.
