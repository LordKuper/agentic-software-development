...
7. **Parallel dispatch** — every reviewer delegated to as a **fresh agent** each iteration (clean-context dispatch per `review-policy.md`); no reviewer reused across iterations:
   ...
   - each reviewer returns its findings + complete coverage ledger (file, rule, and section) + verdict as final text per `t_review.md` (or `external-review/t_review-report.md` for external), first-line verdict token `[REVIEW-design-<reviewer>]: ...`; **this workflow writes the validated compact coverage evidence of that text to `<sprint>/reviews/design/iter-NN/<reviewer>.md`** (step 8) — the reviewer itself performs no write
...
8a. **Interrupted dispatch / split dispatch** — internal reviewers only; `review-policy.md` "Interrupted dispatch and split dispatch" is the sole SSoT for trigger, partition, union property and merge rule, not restated here. A dispatch returning no verdict token or no ledger takes the same reject-and-re-dispatch-fresh path as a failed validation above, and counts one interrupted attempt for that reviewer against the manifest digest it carried — counted here, for the current iteration, never as a `state.json` field:
   - **First interruption** → re-dispatch that reviewer fresh on the same manifest, step 7 payload unchanged.
   - **Second consecutive interruption on the same digest** → split: partition the manifest's `files`, build one complete manifest per half (own file half, full `rules`/`sections`, `n_a` filtered to the ids that half keeps), stamp each with `node .asd/runtime.js manifest-digest --manifest <path> --write`, then dispatch each half as its own fresh agent carrying the step 7 payload narrowed to that half. Gate each half's returned ledger with `validate-ledger` exactly as above; a half interrupted twice → request user decision (never a recursive split).
   - **Merge** only after verifying the union property against the unpartitioned manifest; a failure blocks the merge, counts that reviewer incomplete and re-dispatches it as a failed validation does. Write each half to `<sprint>/reviews/design/iter-NN/<reviewer>.part-1.md`/`.part-2.md` (own verdict token, findings, validated ledger) and the merged token plus links to both parts to `<reviewer>.md` — the single path step 9 parses, unchanged for every reader.
