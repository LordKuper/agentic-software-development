...
4. Write `state.json` (phase=plan) inline; the main orchestrator performs:
   ...
   - instruction:
     ...
     - **Task decomposition rules**:
       ...
       - write the Tasks' touched paths, one per line, to a temp file outside the repo; run `node .asd/runtime.js surface-check --files <path>` and declare its `files` as `Change surface: <n> files` in `## Overview`. `breach: true` blocks acceptance until a split or an approved `change-surface cap override` bound (`sprint-lifecycle.md` "Plan file format"). Above `.asd/runtime.js` `SPLIT_THRESHOLD_FILES`, also name the expected part count per internal reviewer in `## Risks`; `emit-manifest` splits on the real scope file list (`review-policy.md` "Split trigger")
