...
**Union property, checked before merge.** Mechanical, over both half manifests and their validated ledgers against the unpartitioned manifest: (a) the halves' file ids are disjoint and union to exactly the unpartitioned `files` list; (b) `rules`/`sections` equal the unpartitioned arrays in both; (c) each half's digest verifies against its own manifest; (d) every rule id and section id resolves non-`n/a` in at least one half — both halves `n/a` means nobody reviewed it. Any check failing blocks the merge — that reviewer counts as incomplete. (a)–(d) are the contract for a `validate-partition --whole <path> --halves <path...>` helper in `.asd/runtime.js`; until it exists the orchestrator runs them by hand over those files.
