...
|---|---|
| design-review | Efficiency (over-engineering + structure/cohesion + complexity-vs-value tradeoff sections), Documentation — both dispatched for any non-empty draft set unless APPROVE-latched (below); Correctness (UI rubric section only) — dispatched, and required, only when the draft set includes a ux-spec or design-system artifact this iteration; otherwise not dispatched at all and not counted toward DoD; External Review (if enabled) |
...
Every internal reviewer above is dispatched in its listed phase(s) unless already APPROVE-latched (below), with one exception: design-review's Correctness is not dispatched at all — no fresh agent call, no review file, not counted toward DoD — on an iteration where no ux-spec/design-system draft is in the set (`asd-phase-design-review.md` step 7); this is an agent-level skip, distinct from the latch. Separately, "Diff-scoped impl-review fan-out" below is a section-level mechanism scoped to impl-review only: it never skips a reviewer's dispatch, only marks a rubric SECTION `n/a` inside a still-dispatched reviewer. External Review counts as one reviewer when `review.external_review: enabled`, scoped to whichever draft set actually exists that iteration (`sprint-lifecycle.md` "Optional documents"). PR-phase DoD checks only the reviewers/sections this table actually required for the sprint's document profile — a reviewer or section never applicable that iteration (design-review's Correctness with no ux-spec/design-system draft; an impl-review section n/a per the diff-derived predicate) is not counted as missing. On DoD met, the phase advances.
