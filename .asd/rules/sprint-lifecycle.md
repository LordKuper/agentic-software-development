...
`derived_handoff` (`t_state.json` schema) holds the one file list a phase derived and hands to the next — a single slot, last writer wins: `{"base": "<sha>", "head": "<sha>", "pathspec": "<the exact pathspec the diff used>", "files": ["<repo-relative path>", …]}`. Written inline by `asd-phase-impl-test.md` step 2 (change surface / re-entry delta) and `asd-phase-impl-review.md` step 2 (step 1's scope file list, same step that records the iteration head), each at the moment it derives its list, `head` = `git rev-parse HEAD` then. A reader uses `files` ONLY when all three of `base`, `head` and `pathspec` equal what it would pass itself; on any mismatch — and on an absent or empty `derived_handoff` (a sprint in flight when this field shipped), treated as `{}` exactly like the `latched` fallback above, never an error — it re-derives by running the diff as before this field existed and overwrites the slot. Purely an optimisation: re-derivable by definition, never a gate or DoD input, so an absent, stale or mismatched record costs one diff and blocks nothing. Any commit — including a phase's own exit bookkeeping commit — moves `HEAD` and invalidates the record; deliberate, this is a same-`HEAD` cache whose hits are within one phase (its own later steps, an interrupted re-dispatch or split at `asd-phase-impl-review.md` step 7a, a session-loss resume). A stale list is never read, so it can never silently narrow a review.
