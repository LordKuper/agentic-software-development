...
- **change** — the edit's own correctness is uncertain: unfamiliar domain, ambiguous judgment, a contract whose right wording is not yet known, security/authentication/migration/public-contract/workflow-gate work. Routes `critical`. Those five classes are reserved: declaring one `artifact` is invalid input, rejected by `route-task`.
...
`derived_handoff` (`t_state.json` schema) — one slot, last writer wins, the file list a phase hands to the next: `{"base": "<sha>", "head": "<sha>", "pathspec": "<the exact pathspec the diff used>", "files": ["<repo-relative path>", …]}`. `base` = the diff's left side resolved (`git rev-parse <ref>`), never a ref name; `head` = the newest commit in `<base>..HEAD` touching `pathspec` (`git log -1 --format=%H <base>..HEAD -- <pathspec>`, empty when none), never raw `HEAD`, so a bookkeeping commit never invalidates the record.
Written at derivation time (`asd-phase-impl-test.md` and `asd-phase-impl-review.md`, both step 2) as an intra-phase cache, and re-recorded at phase exit — the same diff re-run in the final bookkeeping write, so it covers the in-scope commits (authored tests, review fixes) landing after step 2. The next phase reads the exit record.
A reader uses `files` ONLY when `base`, `head` and `pathspec` all equal what it would pass itself; otherwise — absent, `{}`, or shape-invalid (not an object, or `files` missing or not an array) all count as `{}`, never an error — it re-derives by running the diff and overwrites the slot.
Pure optimisation: never a *verdict* or DoD input, and may only reproduce a list the reader would derive identically.
