...
`derived_handoff` (`t_state.json` schema) — one slot, last writer wins, the file list a phase hands to the next: `{"base": "<sha>", "head": "<sha>", "pathspec": "<the exact pathspec the diff used>", "files": ["<repo-relative path>", …]}`. `base` = the diff's left side resolved (`git rev-parse <ref>`), never a ref name; `head` = the newest commit in `<base>..HEAD` touching `pathspec` (`git log -1 --format=%H <base>..HEAD -- <pathspec>`, empty when none), never raw `HEAD`. Written once, at `asd-phase-impl-test.md` step 10's green exit, re-running that phase's scope diff before its bookkeeping commit — which carries the write and, touching only out-of-pathspec bookkeeping paths, leaves `head` valid. Read once, by `asd-phase-impl-review.md` step 1. One writer, one reader, one handoff edge: no other phase writes or reads the slot. A reader uses `files` ONLY when `base`, `head` and `pathspec` all equal what it would pass itself; otherwise — absent, `{}`, or shape-invalid (not an object, or `files` missing or not an array) all count as `{}`, never an error — it re-derives by running the diff, without writing back. Pure optimisation: never a *verdict* or DoD input, and may only reproduce a list the reader would derive identically.
