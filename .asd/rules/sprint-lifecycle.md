...
**Dispatch ceiling.** One phase step runs at most `.asd/runtime.js` `DISPATCH_CEILING` agent dispatches concurrently; above it, the step dispatches in order in sequential waves of at most that many — review parts and an impl task wave (as sub-waves) alike. Correlated interruption (`review-policy.md` "Correlated interruption") is judged per wave: only dispatches in flight together share one cause.
...
- External Review's partial outcome, `"APPROVE (partial: <n>/<m> files; <cause>)"` (`external-review.md` "Outcome contract") — satisfied for its iteration, never latched (same carve-out);
