...
- A dispatched `asd-dev`/`asd-tester` commit carries one trailer line `ASD-Task: <id>` per id it covers — `Task N`, a review finding id (its id in the reviewer's ledger `findings`, prefixed by its review file when another file of the iteration reuses it: `correctness.part-2.md COR-1`), `D-N`, `impl-test entry N` for impl-test's own commits, or `impl-review iter-NN suite` for impl-review step 9's in-place test fix
...
- `gh` is the only PR path (`/asd-init` requires it). `gh` itself failing — not installed, not authenticated, host unreachable — is `FAILED` naming the fix: install gh (https://cli.github.com) or `gh auth login`. Never a manual-PR fallback.
