...
| 004-review-scoping-and-test-audit | `.claude/agent-memory/asd-reviewer-quality/` | (accepted-debt) leftover agent-memory dir for a reviewer retired this sprint (merged into asd-reviewer-correctness, no 1:1 name successor); same out-of-scope reasoning as the row above | asd-dev |
| 004-review-scoping-and-test-audit | `.claude/agent-memory/asd-backend-dev/` | (accepted-debt) leftover agent-memory dir for a dev agent retired this sprint (merged with asd-frontend-dev into asd-dev, which already holds its own fresh memory — no clean 1:1 successor to fold into); same out-of-scope reasoning as the row above | asd-dev |
