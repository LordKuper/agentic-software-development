...
cd my-project
rm CLAUDE.md README.md   # these belong to the ASD repo, not your project
```

> **Do not copy this repo's `CLAUDE.md` into your project.** It documents how to develop the ASD framework itself and is meaningless in a consumer project. Your project's `CLAUDE.md` is generated by `/asd-init` from `.asd/templates/t_CLAUDE.md` — do not create it by hand.
...
```bash
node .claude/skills/asd-update/update.js --dry-run
```

That command is also the manual fallback if you prefer running it outside Claude Code. It needs `tar` on PATH (bundled with Windows 10 1803+, macOS, Linux) and Node >= 16.7.

> **After updating, reconcile `.claude/settings.json` yourself.** It holds your permission allowlist and hook registration, so the updater never overwrites it. If an update added a hook or skill, you may need to register it there manually.
...
| `/asd-update` | Update framework infrastructure (rules, templates, ASD agents/skills/hooks) to the latest version from the ASD repo's main branch; never touches your config, sprints, design docs, or custom skills/agents/hooks |
