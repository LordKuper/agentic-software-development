...
The main orchestrator owns scope, plan, state, decisions, manual-step validation, Git and release/archival sequencing; no PM agent is spawned. Dev/Tester task variants share each canonical role body and permissions: `-mechanical` uses Haiku (no effort)/Luna low, `-standard` Sonnet/Terra medium, `-critical` Opus/Sol high. Deterministic bookkeeping uses commands. Routing uses objective eligibility, escalates on risk or failed checks, and never changes the main model. Experimental cheap outputs retain strong independent review.
...
| `asd-external-review` | sonnet/medium | terra/medium | both | Wraps the *other* provider's CLI (Codex CLI under Claude Code, Claude CLI under Codex), reads its own content from a structured scope manifest (files/commits + refs, never a rendered diff), parses output, applies severity floor |
...
Yes — set `self_hosting: enabled` in `config.yaml` (this repo ships with it enabled, `documents.audit` only). `/asd-sprint` then edits ASD's own canonical sources per the exhaustive write allowlist in `.asd/rules/sprint-lifecycle.md` "Self-hosting" — generated `.claude/`/`.codex/`/`.agents/skills/` stay off-limits, resynced via `node .asd/sync.js --apply` after every canon edit. Root `AGENTS.md`'s managed block generates from `t_AGENTS.md` like any consumer project; this repo's framework-dev prose lives below `<!-- asd:end -->`, outside the block, as an explicit override. `/asd-update` refuses to run here (it pulls framework files INTO a consumer; a self-hosting repo IS the framework).
