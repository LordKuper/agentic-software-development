...
- **Phase chain**: in `session-start.js` (`PHASE_CHAIN`), `sprint-lifecycle.md`, `core.md` glossary, README, `checkpoints.md`, `asd-sprint/SKILL.md`, `asd-phase-impl-review.md`, `review-policy.md`, and `asd-phase-impl-review/SKILL.md` — all list the same eleven phases in the same order. A skill description carries a chain assertion; a workflow return contract carries a chain assertion; a phase table or enumeration carries a chain assertion — every location bearing any must be updated together when the chain changes.
